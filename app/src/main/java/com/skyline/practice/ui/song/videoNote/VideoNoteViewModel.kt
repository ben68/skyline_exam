...
class VideoNoteViewModel(private val repository: AppRepository): BaseNoteViewModel(repository) {
    ...
    fun saveVideoUriToDatabase(songId: String, fileUri: Uri?) {
        isLoading.value = true
        (songNote.value?: SongNote(songId = songId))
            .apply {
                videoUri = fileUri.toString()
                observe {
                    Completable
                        .fromAction {
